...
# Convert NaN -> None (only for object columns)
df_hungry['name'] = df_hungry['name'].astype(object).where(pd.notnull(df_hungry['name']), None)
...
""", conn2).head(4).reset_index(drop=True)
